...
const HERO_ROTATE_INTERVAL_SEC = 1.5;
...
export default function LandingPage() {
  ...
  React.useEffect(() => {
    const timer = setInterval(() => setFeedIdx((i) => i + 1), 3200);
    return () => clearInterval(timer);
  }, []);

  const liveFeed = [
    { emoji: '🎉', accent: '#38bdf8', region: '서울 강남구', action: '새 타일을 점령했다' },
    { emoji: '🔥', accent: '#f472b6', region: '부산 해운대구', action: '영토가 빠르게 확장 중' },
    { emoji: '🏃', accent: '#34d399', region: '한강 러닝 코스', action: '러너가 지도를 채우는 중' },
    { emoji: '✨', accent: '#a78bfa', region: '제주 앞바다', action: '먼 바다 타일을 원격 점령' },
    { emoji: '📸', accent: '#fbbf24', region: '경북 경주시', action: '점령지에 사진이 올라왔다' }
  ];
  const currentFeed = liveFeed[feedIdx % liveFeed.length];
  const previousFeed = liveFeed[(feedIdx + liveFeed.length - 1) % liveFeed.length];
    ...
    <div className="premium-landing-root">
          ...
          <div className="hero-inner">
            <div className="hero-content">
              ...
              <h1 className="hero-title">
                <span className="word-rise" style={{ animationDelay: '.05s' }}>지루했던</span>
                <span className="word-gap" />
                <span className="word-rise" style={{ animationDelay: '.17s' }}>일상을</span>
                <br />
                <span className="word-rise" style={{ animationDelay: '.32s' }}>
            ...
            <div className="hero-visual animate-fade-in">
              <div className="hud-chip hud-gold">💰 +12 GOLD</div>
              <div className="hud-chip hud-tile">📍 새 타일 점령!</div>
              <div className="map-view-card">
                ...
                {heroImages.map((src, index) => (
                  <img
                    ...
                    style={{
                      animationDelay: `${index * HERO_ROTATE_INTERVAL_SEC}s`,
                      animationDuration: `${HERO_ROTATE_INTERVAL_SEC * heroImages.length}s`
                    }}
                  />
                ))}
              ...
              <div className="live-feed">
                <div key={`prev-${feedIdx}`} className="feed-toast feed-prev">
                  <span className="feed-dot" style={{ background: previousFeed.accent }} />
                  <span>{previousFeed.emoji} <b>{previousFeed.region}</b> {previousFeed.action}</span>
                </div>
                <div key={`cur-${feedIdx}`} className="feed-toast feed-current" style={{ '--fa': currentFeed.accent }}>
                  <span className="feed-dot" style={{ background: currentFeed.accent }} />
                  <span>{currentFeed.emoji} <b>{currentFeed.region}</b> {currentFeed.action}</span>
                </div>
              </div>
            </div>
          </div>
          <div className="scroll-cue">
            <ChevronDown size={16} className="cue-icon" />
            <span>스크롤해서 더 알아보기</span>
          </div>
          <div className="hero-mid" aria-hidden="true">
            <p className="hero-mid-line">걷고, 뛰고, 머무는 모든 순간이</p>
            <p className="hero-mid-line hero-mid-accent">나만의 영토가 됩니다</p>
          </div>
        </div>
      </section>

      <section className="marquee-section" data-reveal>
        <div className="marquee">
          <div className="marquee-track">
            {[...identity, ...identity, ...identity, ...identity].map((it, i) => (
              <div key={`${it.text}-${i}`} className="identity-item">
      ...
      <section className="features-section">
        ...
        <div className="feature-rows">
          {rows.flatMap((row) => row.items).map((it, i) => (
            <div key={it.title} className="mini-feature" data-reveal style={{ '--rd': `${i * 0.09}s` }} onMouseMove={spotlight}>
      ...
      <section className="systems-section">
        ...
        <div className="systems-grid">
          {systems.map((s, i) => (
            <div key={s.title} className="system-card" data-reveal style={{ '--rd': `${i * 0.12}s` }} onMouseMove={spotlight}>
      ...
      <section className="how-section">
        ...
        <div className="timeline">
          {steps.map((s, i) => (
            <div key={s.no} className="tl-node" data-reveal style={{ '--rd': `${i * 0.14}s` }}>
      ...
      <section className="cta-section">
        <div className="cta-panel" data-reveal>
          ...
          <div className="download-badges center">
            ...
            <a href="https://apps.apple.com/kr/app/%EC%B0%9C-%EB%8C%80%EB%AA%A8%ED%98%98/id6769717240" target="_blank" rel="noopener noreferrer" className="badge-link">
              <img src="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg" alt="Download on the App Store" className="badge-img" />
            </a>
          </div>
        </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .premium-landing-root{background-color:#05070f;background-image:radial-gradient(1100px 620px at 12% -12%,rgba(56,189,248,.10),transparent 60%),radial-gradient(1000px 560px at 100% 6%,rgba(167,139,250,.10),transparent 55%),radial-gradient(900px 620px at 50% 115%,rgba(52,211,153,.07),transparent 60%);color:#e8edf6;font-family:'Plus Jakarta Sans',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;min-height:100vh;position:relative;overflow-x:clip;line-height:1.55}
        .logo-text,.hero-title,.section-title,.mini-title,.cta-title,.tl-title{font-family:'Outfit',sans-serif}

        .stars{position:fixed;inset:0;z-index:0;pointer-events:none;background-image:radial-gradient(1px 1px at 12% 22%,rgba(255,255,255,.5),transparent),radial-gradient(1px 1px at 68% 12%,rgba(255,255,255,.35),transparent),radial-gradient(1.5px 1.5px at 84% 44%,rgba(125,211,252,.4),transparent),radial-gradient(1px 1px at 32% 64%,rgba(167,139,250,.4),transparent),radial-gradient(1px 1px at 52% 86%,rgba(255,255,255,.3),transparent),radial-gradient(1.5px 1.5px at 8% 78%,rgba(52,211,153,.35),transparent);background-size:720px 720px;animation:starTwinkle 6s ease-in-out infinite alternate}
        @keyframes starTwinkle{0%{opacity:.45}100%{opacity:.9}}
        ...
        .orb-1{width:540px;height:540px;background:radial-gradient(circle,#38bdf8 0%,transparent 70%);top:-140px;left:12%;animation:orbDriftA 21s ease-in-out infinite alternate}
        .orb-2{width:640px;height:640px;background:radial-gradient(circle,#a78bfa 0%,transparent 70%);top:26%;right:-140px;animation:orbDriftB 26s ease-in-out infinite alternate}
        .orb-3{width:440px;height:440px;background:radial-gradient(circle,#34d399 0%,transparent 70%);bottom:6%;left:-70px;animation:orbDriftC 18s ease-in-out infinite alternate}
        @keyframes orbDriftA{0%{transform:translate(0,0) scale(1)}100%{transform:translate(90px,60px) scale(1.15)}}
        @keyframes orbDriftB{0%{transform:translate(0,0) scale(1.05)}100%{transform:translate(-110px,-70px) scale(.92)}}
        @keyframes orbDriftC{0%{transform:translate(0,0) scale(.95)}100%{transform:translate(120px,-50px) scale(1.12)}}

        .glass-header{position:fixed;left:0;right:0;top:0;z-index:100;background:rgba(5,7,15,.65);backdrop-filter:blur(16px) saturate(140%);-webkit-backdrop-filter:blur(16px) saturate(140%);border-bottom:1px solid rgba(255,255,255,.05)}
        ...
        .hero-section{--hp:0;position:relative;z-index:10;height:260vh}
        .hero-sticky{position:sticky;top:0;height:100vh;display:flex;flex-direction:column;justify-content:center;overflow:hidden}
        .hero-inner{max-width:1200px;width:100%;margin:0 auto;padding:5rem 2rem 0 2rem;display:grid;grid-template-columns:1.05fr .95fr;gap:4rem;align-items:center}
        .hero-content{opacity:calc(1 - var(--hp)*2);transform:translateY(calc(var(--hp)*-90px))}
        ...
        .hero-title{font-size:clamp(3rem,6.5vw,5.4rem);font-weight:700;line-height:1.06;letter-spacing:-.04em;margin:0 0 1.8rem 0}
        .word-rise{display:inline-block;opacity:0;animation:wordRise 1s cubic-bezier(.2,.8,.2,1) forwards}
        .word-gap{display:inline-block;width:.28em}
        @keyframes wordRise{from{opacity:0;transform:translateY(.6em) rotate(2deg);filter:blur(6px)}to{opacity:1;transform:translateY(0) rotate(0);filter:blur(0)}}
        ...
        .hero-desc{font-size:1.2rem;line-height:1.7;color:#9fb0c9;max-width:520px;margin:0 0 2.4rem 0}
        .download-badges{display:flex;gap:1.2rem;align-items:center}
        .download-badges.center{justify-content:center}
        .badge-link{position:relative;overflow:hidden;border-radius:10px;transition:all .3s cubic-bezier(.4,0,.2,1)}
        .badge-link:hover{transform:translateY(-4px) scale(1.02);filter:drop-shadow(0 8px 20px rgba(0,0,0,.5))}
        .badge-link::after{content:'';position:absolute;top:-40%;left:-80%;width:50%;height:180%;background:linear-gradient(105deg,transparent,rgba(255,255,255,.45),transparent);transform:skewX(-20deg);transition:left .6s ease}
        .badge-link:hover::after{left:130%}
        .badge-img{height:52px;display:block}

        .hero-visual{position:relative;display:flex;justify-content:center;transform:translateY(calc(var(--hp)*-36px)) scale(calc(1 + var(--hp)*.08));transition:transform .1s linear}
        ...
        .map-shot-img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;opacity:0;z-index:1;animation-name:shotCycle;animation-timing-function:linear;animation-iteration-count:infinite}
        @keyframes shotCycle{
          0%{opacity:0;transform:scale(1.1)}
          11.11%{opacity:1}
          30%{opacity:1}
          42.22%{opacity:0;transform:scale(1.02)}
          42.23%,100%{opacity:0}
        }
        ...
        .hud-chip{position:absolute;z-index:6;padding:.45rem .85rem;border-radius:14px;background:rgba(10,14,24,.82);border:1px solid rgba(255,255,255,.14);backdrop-filter:blur(10px);font-family:'Outfit',sans-serif;font-size:.82rem;font-weight:700;color:#e2e8f0;box-shadow:0 12px 30px rgba(0,0,0,.5);opacity:0;pointer-events:none}
        .hud-gold{top:16%;right:-4%;color:#fbbf24;animation:hudPop 5s ease-in-out infinite}
        .hud-tile{bottom:22%;left:-4%;color:#7dd3fc;animation:hudPop 5s ease-in-out 2.5s infinite}
        @keyframes hudPop{0%{opacity:0;transform:translateY(14px) scale(.85)}8%{opacity:1;transform:translateY(0) scale(1)}22%{opacity:1;transform:translateY(0) scale(1)}32%{opacity:0;transform:translateY(-12px) scale(.95)}100%{opacity:0}}

        .live-feed{position:absolute;bottom:-2.2rem;left:50%;transform:translateX(-50%);z-index:7;display:flex;flex-direction:column;gap:.45rem;width:max-content;max-width:88vw}
        .feed-toast{display:inline-flex;align-items:center;gap:.55rem;background:rgba(10,14,24,.78);border:1px solid rgba(255,255,255,.12);backdrop-filter:blur(12px);padding:.5rem .9rem;border-radius:14px;font-size:.78rem;font-weight:500;color:#cbd5e1;white-space:nowrap;box-shadow:0 14px 34px rgba(0,0,0,.55)}
        .feed-toast b{color:#f1f5f9;font-weight:700}
        .feed-current{border-color:color-mix(in srgb,var(--fa,#38bdf8) 45%,transparent);box-shadow:0 14px 34px rgba(0,0,0,.55),0 0 18px color-mix(in srgb,var(--fa,#38bdf8) 22%,transparent);animation:feedIn .55s cubic-bezier(.2,.9,.3,1.2)}
        .feed-prev{opacity:.45;transform:scale(.94);filter:blur(.4px)}
        .feed-dot{width:7px;height:7px;border-radius:50%;flex-shrink:0;animation:pulseGlowIcon 1.4s infinite alternate}
        @keyframes feedIn{from{opacity:0;transform:translateY(16px) scale(.92)}to{opacity:1;transform:translateY(0) scale(1)}}

        .scroll-cue{position:absolute;bottom:2rem;left:50%;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;gap:.2rem;color:#64748b;font-size:.75rem;font-weight:600;letter-spacing:.08em;opacity:calc(1 - var(--hp)*5)}
        .cue-icon{animation:cueBob 1.6s ease-in-out infinite}
        @keyframes cueBob{0%,100%{transform:translateY(0)}50%{transform:translateY(6px)}}

        .hero-mid{position:absolute;inset:0;z-index:5;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:0 2rem;pointer-events:none;opacity:clamp(0,calc((var(--hp) - .42)*3.2),1)}
        .hero-mid::before{content:'';position:absolute;inset:0;background:radial-gradient(52% 46% at 50% 50%,rgba(5,7,15,.9),transparent 78%)}
        .hero-mid-line{position:relative;font-family:'Outfit',sans-serif;font-size:clamp(1.9rem,4.2vw,3.4rem);font-weight:700;letter-spacing:-.03em;color:#f1f5f9;margin:0;line-height:1.25;transform:translateY(calc((1 - var(--hp))*26px))}
        .hero-mid-accent{background:linear-gradient(135deg,#38bdf8,#a78bfa,#34d399);-webkit-background-clip:text;WebkitBackgroundClip:text;-webkit-text-fill-color:transparent;background-clip:text}

        .marquee-section{position:relative;z-index:10;max-width:1100px;margin:1rem auto 0;border-top:1px solid rgba(255,255,255,.05);border-bottom:1px solid rgba(255,255,255,.05);background:rgba(5,7,15,.4)}
        ...
        .marquee-track{display:flex;width:max-content;padding:1.4rem 0;animation:marqueeMove 24s linear infinite}
        .marquee:hover .marquee-track{animation-play-state:paused}
        @keyframes marqueeMove{from{transform:translateX(0)}to{transform:translateX(-50%)}}
        .identity-item{display:flex;align-items:center;gap:.6rem;margin-right:3rem;color:#cbd5e1;font-size:.95rem;font-weight:600;white-space:nowrap}
        ...
        [data-reveal]{opacity:0;transform:translateY(30px);transition:opacity .9s cubic-bezier(.2,.7,.2,1),transform .9s cubic-bezier(.2,.7,.2,1);transition-delay:var(--rd,0s)}
        [data-reveal].revealed{opacity:1;transform:translateY(0)}

        .features-section{position:relative;z-index:10;padding:8rem 2rem}
        ...
        .section-title{font-size:clamp(2.2rem,4.5vw,3.4rem);font-weight:700;margin:0;letter-spacing:-.03em;color:#f1f5f9}
        ...
        .mini-feature{position:relative;overflow:hidden;display:flex;gap:1rem;align-items:flex-start;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:16px;padding:1.3rem;transition:transform .35s cubic-bezier(.4,0,.2,1),border-color .35s,background .35s}
        .mini-feature::after{content:'';position:absolute;inset:0;background:radial-gradient(240px circle at var(--mx,50%) var(--my,50%),rgba(56,189,248,.13),transparent 65%);opacity:0;transition:opacity .35s;pointer-events:none}
        .mini-feature:hover{transform:translateY(-4px);border-color:rgba(99,179,237,.22);background:rgba(255,255,255,.05)}
        .mini-feature:hover::after{opacity:1}
        ...
        .systems-section{position:relative;z-index:10;padding:8rem 2rem;background:linear-gradient(to bottom,#05070f,#090d16)}
        .systems-grid{max-width:1100px;margin:0 auto;display:grid;grid-template-columns:repeat(3,1fr);gap:1.5rem}
        .system-card{position:relative;overflow:hidden;background:linear-gradient(160deg,rgba(30,41,59,.35),rgba(15,23,42,.25));border:1px solid rgba(255,255,255,.06);border-radius:20px;padding:2rem;transition:transform .4s cubic-bezier(.4,0,.2,1),border-color .4s,box-shadow .4s}
        .system-card::after{content:'';position:absolute;inset:0;background:radial-gradient(280px circle at var(--mx,50%) var(--my,50%),rgba(167,139,250,.14),transparent 65%);opacity:0;transition:opacity .35s;pointer-events:none}
        ...
        .how-section{position:relative;z-index:10;max-width:1100px;margin:0 auto;padding:7rem 2rem}
        ...
        .tl-circle{width:60px;height:60px;border-radius:50%;margin:0 auto 1rem;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#0b1220,#111a2e);border:1px solid rgba(99,179,237,.35);color:#7dd3fc;box-shadow:0 0 24px rgba(56,189,248,.2);transition:transform .4s cubic-bezier(.4,0,.2,1),box-shadow .4s}
        .tl-node:hover .tl-circle{transform:scale(1.12);box-shadow:0 0 36px rgba(56,189,248,.4)}
        ...
        .cta-panel{position:relative;max-width:1000px;margin:0 auto;text-align:center;padding:4.5rem 2rem;border-radius:32px;background:linear-gradient(160deg,rgba(56,189,248,.10),rgba(167,139,250,.08));border:1px solid rgba(255,255,255,.08);overflow:visible}
        .cta-panel::before{content:'';position:absolute;inset:0;border-radius:32px;background:radial-gradient(600px 300px at 50% -20%,rgba(56,189,248,.25),transparent 70%);pointer-events:none}
        .cta-panel::after{content:'';position:absolute;inset:-2px;border-radius:34px;border:1px solid rgba(56,189,248,.5);animation:ctaRing 3.2s ease-out infinite;pointer-events:none}
        @keyframes ctaRing{0%{opacity:.7;transform:scale(1)}70%,100%{opacity:0;transform:scale(1.035)}}
        .cta-title{position:relative;font-size:clamp(2rem,4vw,2.8rem);font-weight:700;margin:0 0 1rem 0;letter-spacing:-.03em;background:linear-gradient(135deg,#e2e8f0,#a5b4fc);-webkit-background-clip:text;WebkitBackgroundClip:text;-webkit-text-fill-color:transparent;background-clip:text}
        ...
        @media (max-width:968px){
          ...
          .hero-sticky{position:static;height:auto;padding-bottom:3rem}
          .hero-content{opacity:1;transform:none}
          .hero-visual{transform:none;transition:none}
          .scroll-cue{display:none}
          .hero-mid{display:none}
          .hero-inner{grid-template-columns:1fr;text-align:center;padding:4rem 1.5rem 0 1.5rem;gap:3rem}
          .tag-badge{margin-left:auto;margin-right:auto}
          .hero-desc{max-width:100%}
          .download-badges{justify-content:center;flex-wrap:wrap}
          ...
          .live-feed{position:static;transform:none;margin:2.4rem auto 0;align-items:center;width:auto;max-width:100%}
          .feed-prev{display:none}
          .feed-toast{white-space:normal;text-align:center}
          .identity-item{font-size:.88rem}
          .timeline{grid-template-columns:1fr;gap:2.5rem}
          .timeline::before{display:none}
          .systems-grid{grid-template-columns:repeat(2,1fr)}
          .features-section,.systems-section{padding:5rem 1.5rem}
          .cta-panel::after{display:none}
        }

        @media (prefers-reduced-motion:reduce){
          .premium-landing-root *{animation-duration:.01ms !important;animation-iteration-count:1 !important;transition-duration:.01ms !important}
          .premium-landing-root{--hp:0}
          .hero-section{height:auto}
          .hero-sticky{position:static;height:auto}
          [data-reveal]{opacity:1;transform:none}
          .hero-mid{display:none}
          .map-shot-img{animation:none !important}
          .map-shot-img:not(:first-of-type){opacity:0}
          .map-shot-img:first-of-type{opacity:1}
        }
      ` }} />
    </div>
  );
