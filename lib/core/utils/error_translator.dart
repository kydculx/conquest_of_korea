...
class ErrorTranslator {
  ...
  static String translate(dynamic error) {
    ...
    // 데이터베이스 관련 키워드 통합 처리
    if (lowerMsg.contains('postgrest') ||
    ...
    if (lowerMsg.contains('signup requires a valid password') ||
    ...
    if (lowerMsg.contains('sign_in_failed') ||
        lowerMsg.contains('api_exception: 10') ||
        lowerMsg.contains('apiexception: 10')) {
      return GameStrings.errorGoogleConfig;
    }
    if (lowerMsg.contains('network_error') ||
        lowerMsg.contains('socketexception') ||
        lowerMsg.contains('network error')) {
      return GameStrings.errorNetwork;
    }
    if (lowerMsg.contains('google sign in aborted') ||
        lowerMsg.contains('sign_in_canceled') ||
